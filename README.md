...
-Link to the original dataset: https://archive.ics.uci.edu/dataset/938/regensburg+pediatric+appendicitis
...
After training and selecting the best-performing machine learning model, we focused on interpreting the model's predictions. In medical applications, understanding why a model produces a specific prediction is essential in order to ensure transparency and reliability. For this purpose, we used SHAP (SHapley Additive exPlanations), a widely used explainability method for machine learning models.

### SHAP Overview

SHAP is based on concepts from cooperative game theory. It assigns an importance value to each feature by estimating how much that feature contributes to the model's prediction.

In other words, SHAP explains the prediction of a model by computing how the presence or absence of each feature affects the final output. This allows us to understand which clinical variables have the greatest influence on the diagnosis predicted by the model.

Because our model is used in a medical context, SHAP helps make the results more interpretable for clinicians and data scientists.

### Model Explanation Process

Once the best model was selected and saved, we loaded it and applied the SHAP library to compute feature contributions. The explainer calculates SHAP values for each observation in the dataset, which represent the contribution of each clinical variable to the predicted diagnosis.

These SHAP values allow us to quantify the importance of different patient characteristics such as laboratory results or symptoms in predicting appendicitis.

### SHAP Summary Plot

To visualize the global importance of the variables, we generated a SHAP summary plot.

This plot displays all the features ranked by their impact on the model predictions. Each point represents an individual patient observation, and the color indicates the feature value (from low to high).

This visualization helps identify which clinical variables have the strongest influence on the model's decisions across the entire dataset.

### SHAP Bar Plot

In addition to the summary plot, we generated a SHAP bar plot, which provides a simpler representation of feature importance.

The bar plot displays the average absolute SHAP value of each feature, allowing us to rank variables according to their overall contribution to the model's predictions.

This representation makes it easier to quickly identify the most influential clinical factors involved in the appendicitis diagnosis.

### Integration into the Application

To make the model explanations accessible to users, the SHAP visualizations were integrated into the Streamlit application interface.

When the application runs, users can view both the prediction generated by the model and the corresponding explanation through the SHAP visualizations. This allows users to better understand how different patient characteristics influence the prediction.

### Saving the Explanation Results

The SHAP visualizations are automatically generated and saved as image files during the evaluation phase:

outputs/shap_summary.png

outputs/shap_bar.png

These files are then displayed in the application interface and can also be used for reporting or further analysis.

### Prompt Engineering

During the explainability phase, prompt engineering was used to better understand how to apply SHAP to our machine learning model.

An initial prompt was used to explore how SHAP can be implemented to interpret predictions in a medical classification model. The prompt focused on understanding how SHAP values are computed and how they can be used to identify the contribution of each clinical feature to the final prediction.

The response helped clarify the role of SHAP in explaining model behavior and suggested different types of visualizations, such as the summary plot and the bar plot, which provide insights into the importance of clinical variables.

Another prompt was used to determine how to integrate SHAP explanations into our workflow and visualize them effectively. Based on these suggestions, we generated two main visualizations: a SHAP summary plot to analyze global feature importance across the dataset and a SHAP bar plot to rank variables according to their average impact on model predictions.
